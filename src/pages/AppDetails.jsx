...
const AppDetails = () => {
  ...
  useEffect(() => {
    document.title = `${app.name} | GeminiApp`;
  }, [app.name]);

  const [install, setInstall] = useState(false);
  const [everInstall, setEverInstall] = useState(false);
  ...
  const handleInstall = () => {
    if (!install) {
      ...
      Swal.fire({
        title: "Uninstalled",
        text: `${name} has been uninstalled.`,
        icon: "warning",
        timer: 1500,
        showConfirmButton: false,
        position: "top-end",
  ...
  return (
    <>
      <Helmet>
        <title>{name} | GeminiApp</title>
        <meta name="description" content={`Details and reviews for ${name}`} />
      </Helmet>
      <div className="max-w-5xl mx-auto p-6 space-y-6 bg-gray-900 text-gray-100 rounded shadow-lg">
        ...
        <div className="bg-gray-800 p-4 rounded-lg">
          ...
          {!everInstall ? (
            <p className="text-gray-400">
              You must install the app at least once to submit a review.
            </p>
          ) : (
            <>
              <textarea
